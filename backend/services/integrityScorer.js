...
const { analyzeGitignore } = require("./gitignoreAnalyzer");

// ──────────────────────────────────────────────────────────────────────

function computeIntegrity(commits, hackathon, teamData = {}) {
  const report = {
    ...
    // ── 6. Gitignore & Dependency Analysis ────────────────
    gitignoreAnalysis: {
    ...
    timestampVerification: {
      ...
      firstCommitHash: "",
      firstCommitMessage: "",
      isFirstCommitBeforeStart: false,
      preStartCommits: [],
      preStartCount: 0,
  ...
  // ─────────────────────────────────────────────────────────────────
  // 6. GITIGNORE & DEPENDENCY VALIDATION
  //
  // Analyzes: gitignored files that were still committed (violations),
  // suspicious .gitignore patterns that hide source code,
  // dependency mismatches.
  // Score impact → deducted from metricPadding score.
  // ─────────────────────────────────────────────────────────────────
  const gitignoreContent = teamData?.gitignoreContent || null;
  const repoTree = teamData?.repoTree || [];
  const gitAnalysis = analyzeGitignore(gitignoreContent, sorted, repoTree);

  report.gitignoreAnalysis = {
    gitignorePresent: gitAnalysis.gitignorePresent,
    suspiciousPatterns: gitAnalysis.suspiciousPatterns,
    violations: gitAnalysis.violations,
    totalViolatingFiles: gitAnalysis.totalViolatingFiles,
    violationsByCommit: gitAnalysis.violationsByCommit,
    sourceFilesInGitignore: gitAnalysis.sourceFilesInGitignore,
    trackedFilesMatchingGitignore: gitAnalysis.trackedFilesMatchingGitignore,
    details: [],
  };

  // Flag suspicious gitignore patterns (hiding source code)
  if (gitAnalysis.suspiciousPatterns.length > 0) {
    report.metricPadding.score = Math.max(0, report.metricPadding.score - 6);
    for (const sp of gitAnalysis.suspiciousPatterns) {
      report.flags.push(`🔍 ${sp.reason}`);
      report.gitignoreAnalysis.details.push({ issue: sp.reason });
    }
  }

  // Flag committed files that match gitignore (violations)
  if (gitAnalysis.totalViolatingFiles > 0) {
    const violationPenalty = Math.min(6, Math.ceil(gitAnalysis.totalViolatingFiles / 3));
    report.metricPadding.score = Math.max(0, report.metricPadding.score - violationPenalty);
    report.flags.push(
      `🚨 ${gitAnalysis.totalViolatingFiles} file(s) committed that match .gitignore patterns — suspicious dependency/build file inclusion`
    );
    report.gitignoreAnalysis.details.push({
      issue: `${gitAnalysis.totalViolatingFiles} files were committed despite matching .gitignore rules — these should have been ignored`,
    });
  }

  // Flag commits with high violation ratios
  const highViolationCommits = gitAnalysis.violationsByCommit.filter(
    (v) => v.violationRatio > 50
  );
  if (highViolationCommits.length > 0) {
    report.metricPadding.score = Math.max(0, report.metricPadding.score - 4);
    report.flags.push(
      `🚨 ${highViolationCommits.length} commit(s) have >50% gitignored files — likely storing ignored artifacts in repo`
    );
  }

  // Flag tracked files matching gitignore in repo tree
  if (gitAnalysis.trackedFilesMatchingGitignore.length > 10) {
    report.gitignoreAnalysis.details.push({
      issue: `${gitAnalysis.trackedFilesMatchingGitignore.length} files currently tracked in repo match .gitignore patterns`,
    });
  }

  // ─────────────────────────────────────────────────────────────────
  // 7. AUTOMATED TIMESTAMP VERIFICATION
  //
  // Logs the initial commit timestamp and flags pre-hackathon commits.
  // Score impact → enhances visibility of time.score deductions.
  // ─────────────────────────────────────────────────────────────────
  const firstSorted = sorted[0];
  const firstTimestamp = firstSorted ? new Date(firstSorted.timestamp) : null;
  const isBeforeStart = firstTimestamp ? firstTimestamp.getTime() < hStart : false;

  const preStartCommits = sorted.filter(
    (c) => new Date(c.timestamp).getTime() < hStart
  );

  report.timestampVerification = {
    firstCommitTimestamp: firstTimestamp ? firstTimestamp.toISOString() : null,
    firstCommitAuthor: firstSorted?.author || "",
    firstCommitHash: firstSorted?.commitHash || "",
    firstCommitMessage: firstSorted?.message || "",
    isFirstCommitBeforeStart: isBeforeStart,
    preStartCommits: preStartCommits.map((c) => ({
      hash: c.commitHash,
      message: c.message,
      author: c.author,
      timestamp: new Date(c.timestamp).toISOString(),
      timeBeforeStart: formatTimeDiff(hStart - new Date(c.timestamp).getTime()),
    })),
    preStartCount: preStartCommits.length,
    hackathonStartTime: new Date(hStart).toISOString(),
    hackathonEndTime: new Date(hEnd).toISOString(),
    details: [],
  };

  // Enhanced timestamp flags
  if (isBeforeStart) {
    report.timestampVerification.details.push({
      issue: `First commit by "${firstSorted.author}" was made ${formatTimeDiff(hStart - firstTimestamp.getTime())} BEFORE hackathon started — strong indicator of pre-built code`,
    });
    // Additional time score penalty for first commit being pre-start
    report.time.score = Math.max(0, report.time.score - 3);
  }

  if (preStartCommits.length > 1) {
    report.timestampVerification.details.push({
      issue: `${preStartCommits.length} commits were made before the official start time — code was being developed before the hackathon`,
    });
    report.flags.push(
      `⏰ ${preStartCommits.length} commit(s) timestamped BEFORE official hackathon start — pre-built code detected`
    );
  }

  // ─────────────────────────────────────────────────────────────────
  // 8. OVERALL SCORE, VERDICT & REPO STATUS
  // ─────────────────────────────────────────────────────────────────
  report.totalScore =
    report.time.score +
  ...
  const criticalFlags = report.flags.filter(
    (f) =>
      ...
      f.includes("BEFORE official hackathon") ||
      f.includes("gitignored files") ||
      f.includes("gitignore patterns")
  ).length;
  ...
  if (report.gitignoreAnalysis.totalViolatingFiles > 0)
    summaryParts.push(`${report.gitignoreAnalysis.totalViolatingFiles} gitignore violations`);
  if (report.gitignoreAnalysis.suspiciousPatterns.length > 0)
    summaryParts.push(`suspicious .gitignore patterns detected`);
  if (report.timestampVerification.isFirstCommitBeforeStart)
    summaryParts.push("first commit before hackathon start");
