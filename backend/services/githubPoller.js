...
const { fetchGitignore, fetchRepoTree } = require("./gitignoreAnalyzer");
...
async function deepSyncRepoCommits(team, hackathon) {
  ...
  // ── Fetch .gitignore & repo tree for dependency validation ──
  try {
    const [gitignoreContent, repoTree] = await Promise.all([
      fetchGitignore(repoFullName),
      fetchRepoTree(repoFullName),
    ]);
    await Team.findByIdAndUpdate(team._id, {
      gitignoreContent: gitignoreContent || null,
      gitignoreAnalyzedAt: new Date(),
      repoTree: repoTree || [],
    });
    console.log(
      `[deep-sync] ${repoFullName}: .gitignore ${gitignoreContent ? "found" : "not found"}, tree: ${repoTree.length} files`
    );
  } catch (gitErr) {
    console.warn(`[deep-sync] Gitignore/tree fetch failed for ${repoFullName}:`, gitErr.message);
  }
